...
def press_key():
    ...
    if keyboard.is_pressed('q'):
        ...
        return
    keyboard.press('1')
    # Schedule the next key press in 2 seconds
    timer = threading.Timer(3, press_key)
    timer.start()

# Start the first key press immediately
def on_s_pressed():
    global timer
    # Stop the press_key() function
    if timer:
        timer.cancel()
        print("Stopping press_key() function")
def main():
    ...
    use_brect = True
        ...
        #cv.putText(debug_image, ' '.join(sen), (3, 30),
        #2cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv.LINE_AA)
    # Camera preparation
    ...
    s=""
    press_key()
    keyboard.add_hotkey('q', on_s_pressed)
    while True:
        fps = cvFpsCalc.get()
        if cv.waitKey(10) & 0xFF == ord('q'):
            myobj = gTTS(text=word, lang='en', slow=True)
        ...
        cv.putText(debug_image,word, (3, 30),
...
def draw_info_text(image, brect, handedness, hand_sign_text,finger_gesture_textq):
    ...
    #
    # while len(sence)<10:
    #     sence += hand_sign_text
    #     print(sence)
    #     time.sleep(0.01)
    #
    #     while len(sence)==5:   #new gesture
    #         sen += sence
    #         print(sen)
    #         time.sleep(0.01)
    #
...
# def final_word(image,hand_sign_text):
#     sentence=[]
#     while len(sentence)<100:
#         if len(sentence) > 0:
#             if hand_sign_text != sentence[-1]:
#                 sentence.append(hand_sign_text)
#         else:
#             sentence.append(hand_sign_text)
#
#         if len(sentence) > 5:
